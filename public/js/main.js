...
function photoTile(photoUrl, alt, credit, fallbackTileClass, fallbackIcon, galleryName, galleryIndex) {
  ...
  // Blurred, zoomed-in copy of the same photo behind it, so any leftover
  // space around the full (uncropped) photo is filled with a soft
  // continuation of the image itself instead of a flat color bar - looks
  // the same (and intentional) no matter the photo's original aspect ratio.
  const backdrop = el('img', 'tour-photo-backdrop');
...
// any item in server/content/*.json and it'll show up here automatically -
// no code changes needed. Local photos go in public/images/ and are
